...
text = 'ParamLong."iNSet';
t = strfind(info,text) + length(text);
nSet = 1;
if isempty(t) == 0
   ss = sscanf(info(t:t+35),'%s');
   if isempty(str2num(ss(isstrprop(ss,'digit')))) == 0
      nSet = str2num(ss(isstrprop(ss,'digit')));
   end
end
...
if PATMode == 1
    nReadout = nAverage*nPhase*nRepetition*nContrast*Nsl*Nz*Nc*Ny*nSet;
elseif PATMode == 2 & PATRefScanMode == 2
    ...
    nReadout = nAverage*nPhase*nRepetition*nContrast*Nsl*Nz*Nc*NyPAT*nSet;
end
   
if removeOS == 1
    kSpace = zeros(nAverage, nPhase, nRepetition, nContrast, nSet, Nsl, Nc, Nz, Nx, Ny, 'single');
else
    kSpace = zeros(nAverage, nPhase, nRepetition, nContrast, nSet, Nsl, Nc, Nz, NxOS, Ny, 'single');
end
if readPhaseCorInfo == 1 & nPhCorScan > 0
    kPhaseCor = zeros(nPhCorScan, nPhCorEcho, Nsl, nRepetition, Nc, NxOS, 'single');
end
if readNavigator == 1
    nNavigator = nAverage*nPhase*nRepetition*nContrast*Nsl*Nz*Nc*nEPITrain*nNavEK*nSet;
    kNavigator = zeros(nAverage, nPhase, nRepetition, nContrast*nNavEK, nSet, Nsl, Nc, Nz, nEPITrain, NxOS, 'single');
end
if readTimeStamp == 1
   nTimeStamp = nAverage*nPhase*nRepetition*nContrast*Nz*nSet;
   timeStamp = zeros(nAverage, nPhase, nRepetition, nContrast*nNavEK, nSet, Nz, 'single');
end
          ...
          if isempty(find(flag == 22)) == 0 & readPhaseCorInfo == 1
           ...
           if isempty(find(flag == 2)) == 0 & readNavigator == 1
                 if countNavigator == 0 & navigatorPrep == 0
                    kNavigator = single(zeros(Nxr, Ncr, nContrast*nNavEK, nSet, nEPITrain, Nz, Nsl, nAverage, nPhase, nRepetition));
           ...
           if readOneCoil == 0
              if removeOS == 1
                 ...
                 kSpace(Acquisition+1, Phase+1, Repetition+1, Echo+1, Set+1, Slice+1, Channel+1, Partition+1, :, Line+1) = single(tempK);
              else
                 kSpace(Acquisition+1, Phase+1, Repetition+1, Echo+1, Set+1, Slice+1, Channel+1, Partition+1, end-Nxr+1:end, Line+1) = single(temp(1:2:end)+i*temp(2:2:end));
              end
              count = count+1;
              if mod(count,10) == 0
                  count;
           ...
           elseif readOneCoil == 1 & Channel+1 == coilIndex
              if removeOS == 1
                 ...
                 tempX = fftshift(fft(fftshift(temp1)));
                 tempK = fftshift(ifft(fftshift(tempX(round((NxOS-Nx)/2)+1:Nx+round((NxOS-Nx)/2)))));
                 kSpace(Acquisition+1, Phase+1, Repetition+1, Echo+1, Set+1, Slice+1, 1, Partition+1, :, Line+1) = single(tempK);
              else
                 kSpace(Acquisition+1, Phase+1, Repetition+1, Echo+1, Set+1, Slice+1, 1, Partition+1, end-Nxr+1:end, Line+1) = single(temp(1:2:end)+i*temp(2:2:end));
              end
              count = count+1;
           end
           ...
           if readTimeStamp == 1 & Channel == 0 & navigatorDataON == 1
              ...
              timeStamp(Echo+1, EPITrain+1, Partition+1, Set+1, Slice+1, Acquisition+1, Phase+1, Repetition+1) = single(0.0025*timeS);
           end

           if readNavigator == 1 & Channel == 0 & navigatorDataON == 1
              EPITrain = mod(countNavigator, nEPITrain);
              kNavigator(:, :, :, EPITrain+1, Partition+1, Set+1, Slice+1, Acquisition+1, Phase+1, Repetition+1) = single(kNavigatorTemp);
              navigatorDataON = 0;
