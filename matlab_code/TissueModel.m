...
GrayMatter = Tissues.GrayMatter;
CSF = Tissues.CSF;
Cartilage = Tissues.Cartilage;
...
for r = 1:Rows
    for c = 1:Cols
        ...
        elseif sum(squeeze(t(r,c,:)) == white') == 3
            Tissue(r,c) = Fat;
        elseif sum(squeeze(t(r,c,:)) == yellow') == 3
            Tissue(r,c) = GrayMatter;
        elseif sum(squeeze(t(r,c,:)) == blue') == 3
            Tissue(r,c) = Muscle;
        elseif sum(squeeze(t(r,c,:)) == green') == 3
            Tissue(r,c) = CSF;
        elseif sum(squeeze(t(r,c,:)) == cyan') == 3
            Tissue(r,c) = Cartilage;
        else
            Tissue(r,c) = Empty;
        end
    end
