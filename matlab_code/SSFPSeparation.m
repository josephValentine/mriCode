function [ water, fat ] = SSFPSeparation( img1, img2, img3, img4, dixonmode )
    ...
    if dixonmode == 0
        [water, fat] = DixonSeparationInOrder( s{1}, s{2}, s{3}, s{4}, indices{1}, indices{2}, indices{3} );
    elseif dixonmode == 1
        [water, fat] = DixonSeparationInOrderDebug( s{1}, s{2}, s{3}, s{4}, indices{1}, indices{2}, indices{3} );
    else
        [water, fat] = DixonSeparationInOrderDebug2( s{1}, s{2}, s{3}, s{4}, indices{1}, indices{2}, indices{3} );
    end    
...
function [water, fat] = DixonSeparationInOrder( Image1, Image2, Image3, Image4, Index1, Index2, Index3 )
    N = 3;
    [Ny, Nx] = size(Image1);
    water = zeros(Ny, Nx);
    fat = zeros(Ny, Nx);
        ...
        for r = 1:Ny
            for c =1:Nx
                ...
                if( Index(1) && Index(2) && Index(3) )
                    if( (angle(Image1(r,c)) + angle(Image2(r,c))) > (angle(Image2(r,c)) + angle(Image3(r,c))) )
                ...
                elseif( Index(2) && Index(3) && Index(4))
                    if( (angle(Image2(r,c)) + angle(Image3(r,c))) > (angle(Image3(r,c)) + angle(Image4(r,c))) )
                        [water(r,c), fat(r,c)] = DixonSeparation(Image2(r,c),Image3(r,c));
                    else
                        [water(r,c), fat(r,c)] = DixonSeparation(Image3(r,c),Image4(r,c));    
                    end
...
function [water, fat] = DixonSeparationInOrderDebug( Image1, Image2, Image3, Image4, Index1, Index2, Index3 )
    N = 3;
    [Ny, Nx] = size(Image1);
    water = zeros(Ny, Nx);
    fat = zeros(Ny, Nx);
...
function [water, fat] = DixonSeparationInOrderDebug2( Image1, Image2, Image3, Image4, Index1, Index2, Index3 )
    N = 3;
    [Ny, Nx] = size(Image1);
    water = zeros(Ny, Nx);
    fat = zeros(Ny, Nx);
