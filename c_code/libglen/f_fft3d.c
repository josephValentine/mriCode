...
/* 
  ...
  Does 3d fft on rows x cols x planes (float) matrix with real part in re,
...
f_fft3d(re,im,re2,im2,rows,cols,planes,inv)
float *re,*im,*re2,*im2;
...
/* transform planes */
  ...
  a = cols * rows;
  for(i=0; i<planes; i++)
    f_fft2d(re2+i*a,im2+i*a,re2+i*a,im2+i*a,rows,cols,inv);
  
/* transform in 3rd dimension */

  if((rbuf = (float *)malloc(sizeof(float)*planes)) == NULL) {
    perror("malloc failed");
    exit(1);
  }
  if((ibuf = (float *)malloc(sizeof(float)*planes)) == NULL) {
    perror("malloc failed");
    exit(1);
  }
  for(i=0; i<rows; i++)
